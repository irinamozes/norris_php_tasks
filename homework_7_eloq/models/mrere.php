...
class ModelData {
  ...
  public function model() {
      ...
      if (!$login_id_save) {
        $lo = 'Введите логин (не меньше 5 символов):';
        $disabl = "text";
        $login_ar_login = '';
        $login_ar_pass = '';
        $profile_ar_name = '';
        ...
        $login_ar = User::find($login_id_save);
        ...
        //$profile_ar = User::find($login_id_save)->profile()->first();
        $profile_ar = Profile::where('user_id', '=', $login_id_save)->first();
        ...
        if ($_COOKIE['idimg']) {
          ...
          //$id_img = $connection->query($sqlimg);
          //$img_ar = mysqli_fetch_assoc($id_img);
          //$img_ar = Image::where('user_id', '=', $login_id_save)->orderBy('id', 'desc')->first()->img_name;
          $img_ar = Image::where('user_id', '=', $login_id_save)->orderBy('id', 'desc')->first();
